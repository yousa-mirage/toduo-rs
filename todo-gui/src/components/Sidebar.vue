...
<template>
  ...
  >
    ...
    <nav class="sidebar-nav">
        ...
        <div class="nav-group">
            ...
            >
                ...
                <span v-if="!isCollapsed && counts.today" class="nav-count">{{ counts.today }}</span>
            </button>
            <button 
                class="nav-item" 
            ...
            >
                ...
                <span v-if="!isCollapsed && counts.next7" class="nav-count">{{ counts.next7 }}</span>
            </button>
        </div>
        ...
        <div class="nav-group">
            ...
            <button class="nav-item" :class="{ active: isFilterActive('priority:A') }" @click="emit('update:filter', 'priority:A')" title="High Priority">
                <span class="dot-priority dot-a">A</span>
                <span v-if="!isCollapsed" class="nav-label">High Priority</span>
            </button>
            <button class="nav-item" :class="{ active: isFilterActive('priority:B') }" @click="emit('update:filter', 'priority:B')" title="Medium Priority">
                 <span class="dot-priority dot-b">B</span>
                <span v-if="!isCollapsed" class="nav-label">Medium Priority</span>
            </button>
            <button class="nav-item" :class="{ active: isFilterActive('priority:C') }" @click="emit('update:filter', 'priority:C')" title="Low Priority">
                 <span class="dot-priority dot-c">C</span>
                <span v-if="!isCollapsed" class="nav-label">Low Priority</span>
            </button>
        </div>
    </nav>
    ...
    <div class="sidebar-footer">
        <button class="nav-item open-btn" @click="emit('open-path')" title="Open Folder">
        ...
        <button class="btn-toggle" @click="toggleSidebar" title="Toggle Sidebar">
            <span class="nav-icon" v-html="isCollapsed ? icons.chevronRight : icons.chevronLeft"></span>
...
.dot-a { background-color: var(--color-danger); }
.dot-b { background-color: var(--color-priority-b); }
.dot-c { background-color: var(--color-priority-c); }
